...
class AccessController extends BaseController
{
    ...
    private function assignablePermissions(): array
    {
        return db_connect()->table('permissions')
            ->whereIn('module', ['dashboard', 'candidates', 'vacancies', 'departments', 'reports', 'recruitment_settings'])
            ->where('is_active', 1)
            ->orderBy("CASE module WHEN 'dashboard' THEN 1 WHEN 'candidates' THEN 2 WHEN 'vacancies' THEN 3 WHEN 'departments' THEN 4 WHEN 'reports' THEN 5 ELSE 6 END", '', false)
            ->orderBy('name', 'ASC')
            ->get()
            ->getResultArray();
    }
