            ...
            <section class="candidate-profile-card <?= $isBlacklistActive ? 'is-blacklisted' : 'is-clear' ?>">
                ...
                <div class="candidate-profile-actions">
                    ...
                    <div class="candidate-profile-action-buttons">
                        ...
                        <?php if ($whatsAppNumber !== ''): ?><?php if ($canUseWhatsappTemplates && $whatsappTemplates !== []): ?><button class="candidate-profile-action candidate-whatsapp-button" type="button" data-admin-modal-open="applicant-whatsapp-modal"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.5 3.5A11.8 11.8 0 0 0 12.1 0C5.6 0 .3 5.3.3 11.8c0 2.1.5 4.1 1.6 5.9L0 24l6.5-1.7c1.7.9 3.6 1.4 5.6 1.4 6.5 0 11.8-5.3 11.8-11.8 0-3.2-1.2-6.2-3.4-8.4Z"/><path d="M8.1 6.8c-.3 0-.7.1-1 .5-.4.4-1.3 1.3-1.3 3s1.3 3.5 1.5 3.7c.2.2 2.5 3.9 6.1 5.3 3 .9 3.7.7 4.4.6.7-.1 2.1-.9 2.4-1.7.3-.8.3-1.4.2-1.6-.1-.2-.4-.3-.8-.5l-2.3-1.1c-.3-.1-.6-.2-.8.2l-1 1.2c-.2.3-.4.3-.8.1-2.1-1-3.5-2.6-4-3.5-.2-.4 0-.6.2-.8l.7-.8c.2-.2.2-.5.3-.7.1-.2 0-.5-.1-.7l-1-2.4c-.2-.6-.5-.8-.7-.8Z"/></svg><span>Kirim WhatsApp</span></button><?php else: ?><a class="candidate-profile-action candidate-whatsapp-button" href="https://wa.me/<?= esc($whatsAppNumber, 'attr') ?>?text=<?= esc(rawurlencode($whatsAppMessage), 'attr') ?>" target="_blank" rel="noopener noreferrer"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.5 3.5A11.8 11.8 0 0 0 12.1 0C5.6 0 .3 5.3.3 11.8c0 2.1.5 4.1 1.6 5.9L0 24l6.5-1.7c1.7.9 3.6 1.4 5.6 1.4 6.5 0 11.8-5.3 11.8-11.8 0-3.2-1.2-6.2-3.4-8.4Z"/><path d="M8.1 6.8c-.3 0-.7.1-1 .5-.4.4-1.3 1.3-1.3 3s1.3 3.5 1.5 3.7c.2.2 2.5 3.9 6.1 5.3 3 .9 3.7.7 4.4.6.7-.1 2.1-.9 2.4-1.7.3-.8.3-1.4.2-1.6-.1-.2-.4-.3-.8-.5l-2.3-1.1c-.3-.1-.6-.2-.8.2l-1 1.2c-.2.3-.4.3-.8.1-2.1-1-3.5-2.6-4-3.5-.2-.4 0-.6.2-.8l.7-.8c.2-.2.2-.5.3-.7.1-.2 0-.5-.1-.7l-1-2.4c-.2-.6-.5-.8-.7-.8Z"/></svg><span>Kirim WhatsApp</span></a><?php endif ?><?php endif ?>
...
<?php if ($whatsAppNumber !== '' && $canUseWhatsappTemplates && $whatsappTemplates !== []): ?>
<dialog class="admin-modal applicant-whatsapp-modal" id="applicant-whatsapp-modal" aria-labelledby="applicant-whatsapp-title">
    <div class="admin-modal-panel" data-applicant-whatsapp-composer data-phone="<?= esc($whatsAppNumber, 'attr') ?>" data-applicant="<?= esc($applicant['full_name'], 'attr') ?>" data-recruiter="<?= esc((string) ($auth['name'] ?? 'Admin HRD'), 'attr') ?>">
        <div class="settings-card-heading admin-modal-heading">
            ...
            <div><h2 id="applicant-whatsapp-title">Kirim WhatsApp ke <?= esc($applicant['full_name']) ?></h2><p>Pesan dapat diedit sebelum WhatsApp Desktop atau Web dibuka.</p></div>
        ...
        <div class="applicant-whatsapp-composer">
            <div class="applicant-whatsapp-selectors">
                <label>Template pesan<select data-applicant-whatsapp-template><?php foreach ($whatsappTemplates as $template): ?><option value="<?= (int) $template['id'] ?>" data-category="<?= esc($template['category'], 'attr') ?>" data-message="<?= esc($template['message_text'], 'attr') ?>" <?= $template['code'] === 'kontak_awal' ? 'selected' : '' ?>><?= esc($template['name']) ?></option><?php endforeach ?></select></label>
                <label>Posisi yang dibahas<select data-applicant-whatsapp-application><?php foreach ($applications as $application): $previousStageName = '-'; $nextStageName = '-'; foreach ($application['process_steps'] as $processStep) { if ($processStep['progress_state'] === 'previous') { $previousStageName = (string) $processStep['name']; } elseif (in_array($processStep['progress_state'], ['next', 'upcoming'], true) && $nextStageName === '-') { $nextStageName = (string) $processStep['name']; } } ?><option value="<?= (int) $application['id'] ?>" data-vacancy="<?= esc($application['vacancy_title'], 'attr') ?>" data-stage="<?= esc($statusLabels[$application['application_status']] ?? ucwords(str_replace('_', ' ', $application['application_status'])), 'attr') ?>" data-previous-stage="<?= esc($previousStageName, 'attr') ?>" data-next-stage="<?= esc($nextStageName, 'attr') ?>"><?= esc($application['vacancy_title']) ?></option><?php endforeach ?></select></label>
                <label>Jadwal terkait (Opsional)<select data-applicant-whatsapp-schedule><option value="" data-application-id="0">Tanpa jadwal</option><?php foreach ($applications as $application): foreach ($schedulesByApplication[(int) $application['id']] ?? [] as $schedule): if ($schedule['status'] === 'cancelled') { continue; } ?><option value="<?= (int) $schedule['id'] ?>" data-application-id="<?= (int) $application['id'] ?>" data-stage="<?= esc($schedule['stage_name'], 'attr') ?>" data-date="<?= esc(date('d M Y', strtotime($schedule['scheduled_at'])), 'attr') ?>" data-time="<?= esc(date('H:i', strtotime($schedule['scheduled_at'])), 'attr') ?>" data-location="<?= esc($schedule['venue'], 'attr') ?>" data-pic="<?= esc($schedule['pic_name'], 'attr') ?>" data-instructions="<?= esc((string) ($schedule['instructions'] ?? ''), 'attr') ?>" data-deadline="<?= esc(date('d M Y, H:i', strtotime($schedule['confirmation_deadline_at'])) . ' WIB', 'attr') ?>"><?= esc($schedule['stage_name']) ?> · <?= esc(date('d M Y, H:i', strtotime($schedule['scheduled_at']))) ?> WIB</option><?php endforeach; endforeach ?></select></label>
            </div>
            ...
            <div class="applicant-whatsapp-hint"><strong>Nomor tujuan</strong><span>+<?= esc($whatsAppNumber) ?> · Pesan belum dikirim otomatis. Periksa kembali lalu tekan Kirim di WhatsApp.</span></div>
...
<script src="<?= base_url('assets/js/admin-hrd.js') ?>?v=13" defer></script>
</body>
</html>
