    ...
    <div class="dashboard-shell">
        <aside class="admin-sidebar" id="admin-sidebar">
            ...
            <nav class="admin-nav" aria-label="Navigasi dashboard HRD">
                ...
                <a class="active" href="<?= site_url('adminhrdmannakampus/akses') ?>">
            ...
            <div class="sidebar-user">
                <span class="user-avatar"><?= esc(mb_strtoupper(mb_substr((string) ($auth['name'] ?? 'S'), 0, 1))) ?></span>
                <span><strong><?= esc($auth['name'] ?? 'Super Admin') ?></strong><small><?= esc($auth['email'] ?? '') ?></small></span>
            </div>
            <form action="<?= site_url('adminhrdmannakampus/logout') ?>" method="post">
                <?= csrf_field() ?>
                <button class="logout-button" type="submit"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 5H5v14h5M14 8l4 4-4 4M8 12h10"/></svg>Keluar</button>
            </form>
        </aside>
