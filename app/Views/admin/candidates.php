...
<head>
    ...
    <link rel="stylesheet" href="<?= base_url('assets/css/admin-hrd.css') ?>?v=16">
</head>
...
<div class="dashboard-shell">
    ...
    <main class="admin-main">
        <header class="admin-topbar">
            ...
            <div><span>Recruitment Pipeline</span><strong>Kandidat</strong></div>
            <a class="view-career-link" href="<?= site_url('adminhrdmannakampus/laporan-pelamar') ?>">Lihat laporan pelamar</a>
        </header>
        ...
        <div class="admin-content candidates-content">
            ...
            <section class="dashboard-welcome department-heading">
                <div><span class="login-eyebrow">Candidate Pipeline</span><h1>Kandidat</h1><p>Proses setiap lamaran melalui tahapan seleksi yang sudah ditetapkan.</p></div>
            ...
            <section class="settings-card candidate-table-card">
                <div class="settings-card-heading settings-heading-action"><span class="settings-icon settings-icon-green"><svg viewBox="0 0 24 24"><path d="M5 6h14M5 12h14M5 18h14"/></svg></span><div><h2>Pipeline kandidat</h2><p>Satu baris mewakili satu lamaran pada posisi tertentu.</p></div><span class="device-count"><?= count($applications) ?></span></div>
                <div class="department-table-wrap">
                    <table class="department-table candidate-table">
                        <thead><tr><th>Kandidat</th><th>Posisi</th><th>Screening</th><th>Tahap saat ini</th><th>Lama di tahap</th><th>Tanggal daftar</th><th>Aksi</th></tr></thead>
                        <tbody>
                            <?php if ($applications === []): ?><tr><td colspan="7" class="department-empty">Kandidat tidak ditemukan.</td></tr><?php endif ?>
                            <?php foreach ($applications as $application): $screening = (string) ($application['screening_status'] ?: 'pending'); ?>
                                <tr>
                                ...
                                <?php if ($canUpdateStatus): ?><tr class="candidate-process-row" id="process-<?= (int) $application['id'] ?>"><td colspan="7"><form class="candidate-process-form" action="<?= site_url('adminhrdmannakampus/kandidat/lamaran/' . $application['id'] . '/tahap') ?>" method="post"><?= csrf_field() ?><div class="candidate-process-title"><strong>Proses <?= esc($application['full_name']) ?></strong><span><?= esc($application['vacancy_title']) ?> · <?= esc($application['application_number']) ?></span></div><label>Tahap baru<select name="stage" required><option value="">Pilih tahapan</option><?php foreach ($stages as $stage): ?><option value="<?= esc($stage['code'], 'attr') ?>" <?= $stage['code'] === $application['application_status'] ? 'disabled' : '' ?>><?= esc($stage['name']) ?></option><?php endforeach ?></select></label><label>Alasan penolakan<select name="rejection_template_id"><option value="">Wajib jika memilih Ditolak</option><?php foreach ($rejectionTemplates as $template): ?><option value="<?= (int) $template['id'] ?>"><?= esc($template['title']) ?></option><?php endforeach ?></select></label><label class="candidate-process-note">Catatan internal<textarea name="notes" rows="3" maxlength="2000" placeholder="Opsional, tersimpan pada riwayat status"></textarea></label><div class="candidate-process-buttons"><a href="<?= site_url('adminhrdmannakampus/kandidat') ?>">Batal</a><button type="submit" data-confirm="Ubah tahapan kandidat ini?">Simpan tahapan</button></div></form></td></tr><?php endif ?>
                            <?php endforeach ?>
                        </tbody>
                    </table>
                </div>
