<?php foreach ($vacancies ?? [] as $vacancy): ?>
    ...
    >
        ...
        <div class="job-opening-details">
            <?php if (! empty(trim((string) ($vacancy['job_description'] ?? '')))): ?><div>
                <h3>Tentang Peran</h3>
                <p><?= nl2br(esc((string) $vacancy['job_description'])) ?></p>
            </div><?php endif ?>
            <?php if ($responsibilities !== []): ?><div><h3>Tugas & Tanggung Jawab</h3><ul><?php foreach ($responsibilities as $item): ?><li><?= esc($item) ?></li><?php endforeach ?></ul></div><?php endif ?>
            <?php if ($qualifications !== []): ?><div><h3>Kualifikasi</h3><ul><?php foreach ($qualifications as $item): ?><li><?= esc($item) ?></li><?php endforeach ?></ul></div><?php endif ?>
